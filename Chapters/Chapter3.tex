...
\begin{itemize}
  ...
  \item Un proveedor para \emph{Java Cryptography Extension} (JCE)\footnote{JCE implementa encriptación, generación y protocolos de establecimiento de claves y algoritmos MAC\index{MAC}.} y para \emph{Java Cryptography Architecture} (JCA).
\end{itemize}
...
Un algoritmo basado en cifrado\index{Cifrado} por bloques es aquel que, como su propio nombre indica, realiza un cifrado\index{Cifrado} primero dividiendo el texto plano en bloques de un tamaño determinado\footnote{Cuando el tamaño del fichero que queremos encriptar no es múltiplo del tamaño de bloque, el bloque final tendrá un tamaño diferente al resto. Esto se soluciona con técnicas de \emph{padding}\index{Padding}, lo cual se explica en el siguiente punto.} y luego cifrando por separado estos bloques, dando como resultado un texto cifrado\index{Cifrado}.
...
El objetivo de un cifrado\index{Cifrado} por bloques es poder paralelizar el proceso de encriptación. Lamentablemente, el cifrado\index{Cifrado} con CBC debe ser secuencial, ya que para obtener cada bloque es necesario haber generado antes el anterior.

Sin embargo, el proceso de desencriptación con CBC sí que puede ser paralelizado, ya que las mútiples funciones de descifrado\index{Descifrado} que se realizan no dependen de ningún bloque anterior. \emph{\parencite{Reference24}}
...
\emph{Advanced Encryption Standard} (AES\index{AES}), también conocido como Rijndael por sus creadores, es el resultado del proceso de búsqueda de un estándar\index{Estándar} para encriptación por parte del gobierno de los Estados Unidos.
...
Si lo que queremos es confidencialidad\index{Confidencialidad} durante la comunicación, entonces encriptaremos el contenido del mensaje con la clave pública\index{Clave pública} del destinatario, de forma que solo él podrá descifrarlo (Figura~\ref{fig:PublicKeyEncryption}).
...
Uno de los primeros algoritmos de criptografía\index{Criptografía} pública\index{Clave pública} (y el más utilizado hoy) es RSA\index{RSA} (Rivest–Shamir–Adleman)\footnote{RSA\index{RSA} debe su nombre a sus creadores: Ron Rivest, Adi Shamir y Leonard Adleman.}. Una de sus características más distintivas es el problema que plantea factorizar el producto de dos grandes números primos (\emph{factoring problem}). El resultado de esta factorización es luego usado en la generación de las claves.

En general, los algortimos de criptografía\index{Criptografía} de clave pública\index{Clave pública} son bastante más lentos que los de clave simétrica\index{Clave simétrica}. Por ello, se suelen usar en conjunto con algún algoritmo de criptografía\index{Criptografía} simétrica\index{Clave simétrica}, como AES\index{AES}. \emph{\parencite{Reference9}}
...
Una característica de HTTP\index{HTTP} es la negociación de la representación de los datos, lo que permite construir sistemas indpendientemente de los datos que se vayan a transmitir. \emph{\parencite{Reference18}}
